...
public class CameraIntentHelper {
    ...
    public CameraIntentHelper(Fragment activity, CameraIntentHelperCallback cameraIntentHelperCallback) {
        mActivity = activity;
...
//				String sdkVersion = android.os.Build.VERSION.RELEASE.toLowerCase(Locale.ENGLISH);
                ...
                if (setPreDefinedCameraUri) {
                    ...
                    preDefinedCameraUri = mActivity.getContext().getContentResolver().insert(
    ...
    private void onCameraIntentResult(int requestCode, int resultCode, Intent intent) {
        if (resultCode == Activity.RESULT_OK) {
            ...
            try {
                ...
                myCursor = mActivity.getContext().getContentResolver().query(MediaStore.Images.Media.EXTERNAL_CONTENT_URI,
                        largeFileProjection,
                        null, null,
                        largeFileSort);
    ...
    private Uri getFileUriFromContentUri(Uri cameraPicUri) {
        ...
        try {
            if (cameraPicUri != null
                ...
                cursor = mActivity.getContext().getContentResolver().query(cameraPicUri, proj, null, null, null);
                cursor.moveToFirst();
                // This will actually give you the file path location of the image.
